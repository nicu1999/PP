...
;(define (get-gravity variables) (variables-gravity variables))
;(define (get-scroll-speed variables) (variables-scroll-speed variables))
...
(define initial-state (state (bird-state bird-initial-y initial-momentum)
                             (list (pipe (+ added-number (random random-threshold)) scene-width))
...
; dintre cele 2 pipe-uri!
 ;(define (make-pipe gap-y x) (pipe gap-y x))
;TODO 16
...
(define (get-initial-state) initial-state)
...
(define (next-state-bird bird gravity)
  (match-let ([(bird-state y v-y) bird]) (struct-copy bird-state bird [y (+ y v-y)] [v-y (+ v-y gravity)]))
)
;TODO 4
...
(define (change current-state pressed-key) ;not done
  (match-let ([(state bird pipe variables abilities score) current-state]) (next-state-bird-onspace (get-bird current-state) initial-momentum))
 )
...
(define (move-pipes pipes scroll-speed)
  (map (lambda (a-pipe) (match-let ([(pipe gap-y x) a-pipe]) (struct-copy pipe a-pipe [x (- x scroll-speed)]))) pipes)
)
...
(define (to-be-deleted pipe)
  (if (< (get-pipe-x pipe) (- pipe-width)) #t #f)
)

(define (clean-pipes pipes)
 (filter to-be-deleted pipes)
)
;TODO 13
...
; cu pasărea.
(define (get-last-pipe pipes)
  (if(null? (cdr pipes)) (car pipes) (get-last-pipe pipes (cdr pipes)))
)

(define (add-more-pipes pipes)
  (if (>=(- (+ (get-pipe-x(get-last-pipe pipes))pipe-width) scene-width) pipe-self-gap)
      (append pipes (pipe (+ added-number (random random-threshold)) scene-width))
      #f)
)
...
(define (check-ground-collision bird)
  (if (>= (+ (get-bird-y bird) bird-height) ground-height)
       #t
       #f
  )
)
...
(define (invalid-state? state)
  (and (check-ground-collision (get-bird state)) (check-pipe-collisions (get-bird state) (state-pipes state)))
)
...
(define (check-pipe-collisions bird pipes)
  (andmap
   ...
   (lambda (pipe)
      (if (and (eq? (check-collision-rectangles
                 (make-posn bird-x (get-bird-y bird))
                 (make-posn (+ bird-x bird-width) (+ (get-bird-y bird) bird-height))
                 (make-posn (get-pipe-x pipe) (get-pipe-gap-y pipe))
                 (make-posn (+ (get-pipe-x pipe) pipe-width) (+ (get-pipe-gap-y pipe) pipe-gap)))
                 #f)
           
                (not (check-collision-rectangles (make-posn bird-x (get-bird-y bird))
                                                (make-posn (+ bird-x bird-width) (+ (get-bird-y bird) bird-height))
                                                (make-posn (get-pipe-x pipe) 0)
                                                (make-posn (+ (get-pipe-x pipe) pipe-width) scene-width))
                )
          )
          #t
          #f
...
(define (next-state a-state)
  (match-let ([(state bird pipes variables abilities score) a-state])
    (struct-copy state a-state
                 [bird (next-state-bird bird (get-gravity variables))]
                 [pipes (next-state-pipes pipes (get-scroll-speed variables))]
                 [score (+ score 0.1)]
)))
